...
function run(){
  ...
  ok('build tag and sw cache version agree', (function () {
       const tag = (q('#buildtag') || {}).textContent || '';
       const sw = require('fs').readFileSync(__dirname + '/sw.js', 'utf8');
       const cv = (sw.match(/const CACHE = '[^']*-v(\d+)'/) || [])[1];
  ...
  ok('refresh reminder offers an off switch',
  ...
  ok('media vocabulary is comic/game/screen',
     JSON.stringify(D.media) === JSON.stringify(['comic','game','screen']));
  ok('no hardcoded franchise chip leaks into Characters', !qa('#strandChips .chip').some(b=>/^ultimate$/i.test(b.textContent)));
  ...
  if (pullBtn) {
    ...
    ok('classic skin has no hardcoded franchise chip', !qa('#strandChips .chip').some(b => /ultimate/i.test(b.textContent)));
